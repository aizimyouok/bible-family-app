...
// 최종 완성본 (모든 기능 포함) - 비전통장 실시간 업데이트 개선 버전
// =================================================================
...
function saveData(params) {
  ...
  Logger.log(`saveData 호출: type=${type}, userId=${userId}, bookName=${bookName}, chapter=${chapter}`);
  ...
  if (type === 'reading') {
    ...
    Logger.log(`가족 시트 확인: headers=${headers}, targetIndex=${targetIndex}`);
    
    if (targetIndex !== -1) {
        for (let i = 1; i < familyData.length; i++) {
            const isTarget = (familyData[i][targetIndex] === true || familyData[i][targetIndex] === 'TRUE');
            Logger.log(`사용자 ${familyData[i][idIndex]} 적립대상 확인: ${isTarget}`);
            
            if (familyData[i][idIndex] == userId && isTarget) {
                const currentUserName = familyData[i][nameIndex] || params.userName || '알 수 없음';
                
                Logger.log(`비전통장 적립 실행: userId=${userId}, name=${currentUserName}`);
                
                // ⭐ addAllowance 함수를 호출하고, 새로 추가된 거래 내역을 반환받음
                const newTransaction = addAllowance(userId, currentUserName, 100, `${bookName} ${chapter}장 읽기`);
...
/**
 * 비전통장 적립
 * (시트에 기록 후, 추가된 행 정보를 객체로 반환하도록 수정됨)
 */
function addAllowance(userId, name, amount, description) {
  ...
  const type = amount > 0 ? '적립' : '차감'; // 적립 또는 차감 타입 설정
  
  const newRow = [transactionId, userId, name, timestamp, type, amount, description];
  sheet.appendRow(newRow);

  // ⭐ 시트에 추가한 행의 정보를 객체로 만들어 반환
  return {
...
function deleteData(params) {
  ...
  switch (type) {
    case 'reading':
      return deleteReading(params.userId, params.bookName, parseInt(params.chapter));
...
// ⭐ 비전통장 실시간 업데이트를 위한 개선된 deleteReading 함수
function deleteReading(userId, bookName, chapter) {
    ...
    try {
        ...
        // (이 부분은 기존과 동일: reading_records에서 읽기 기록을 삭제하는 부분)
        const data = sheet.getDataRange().getValues();
        const rows = data.slice(1);
        const matchingRows = [];
        rows.forEach((row, index) => {
        ...
        if (matchingRows.length === 0) { 
            return { message: '삭제할 기록을 찾지 못했습니다.' }; 
        }
        ...
        // ⭐ --- 비전통장 '적립 기록 삭제' 로직 시작 --- ⭐
        const allowanceSheet = getOrCreateSheet(SHEETS.ALLOWANCE);
        const allowanceData = allowanceSheet.getDataRange().getValues();
        const allowanceHeaders = allowanceData[0];
        
        const allowanceUserIdIndex = allowanceHeaders.indexOf('user_id');
        const allowanceDescIndex = allowanceHeaders.indexOf('description');
        const allowanceTypeIndex = allowanceHeaders.indexOf('type');
        
        // 삭제할 거래내역 설명 (예: "창세기 1장 읽기")
        const descriptionToDelete = `${bookName} ${chapter}장 읽기`;
        
        // ⭐ 삭제된 거래내역 정보를 저장할 변수
        let deletedTransactionInfo = null;

        // 최근 기록부터 거슬러 올라가며 일치하는 거래내역을 찾아서 삭제
        for (let i = allowanceData.length - 1; i >= 1; i--) {
            const row = allowanceData[i];
            const rowUserId = row[allowanceUserIdIndex];
            const rowDescription = row[allowanceDescIndex];
            const rowType = row[allowanceTypeIndex];

            // 사용자ID, 설명, 타입('적립')이 모두 일치하는 첫 번째 기록을 찾으면
            if (rowUserId == userId && rowDescription === descriptionToDelete && rowType === '적립') {
                // ⭐ 삭제하기 전에 정보를 저장
                ...
                allowanceSheet.deleteRow(i + 1); // 해당 행을 삭제
                Logger.log(`'${descriptionToDelete}' 적립 기록을 비전통장에서 삭제했습니다.`);
                break; // 하나만 삭제하고 중단
            }
        }
        // ⭐ --- 비전통장 '적립 기록 삭제' 로직 끝 --- ⭐

        // (이 부분도 기존과 동일: reading_records 시트 최종 정리)
        ...
        sheet.getRange(matchingRows[0].rowIndex, 3, 1, 2).setValues([[JSON.stringify(sortedChapters), JSON.stringify(allReadDates)]]);
        ...
        // ⭐ 삭제된 거래내역 정보와 함께 메시지 반환 (비전통장 실시간 업데이트용)
        return { 
            message: '읽기 기록이 삭제되었습니다.',
            deletedTransactionInfo: deletedTransactionInfo // 클라이언트에서 로컬 데이터 업데이트용
        };
...
// =================================================================
// 파일 끝 - 이 코드를 Google Apps Script 에디터에 복사하여 사용하세요.
// 수정 사항:
